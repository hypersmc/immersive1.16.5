...
import com.jumpwatch.tit.Utils.org.joml.Vector2i;
import com.jumpwatch.tit.Utils.org.joml.Vector3i;
import com.jumpwatch.tit.Utils.org.joml.Vector3ic;
import com.jumpwatch.tit.Utils.AStarList;
import com.jumpwatch.tit.Utils.TileMap;
...
public class MultiblockController<ControllerType extends MultiblockController<ControllerType, TileType, BlockType>, TileType extends MultiblockTile<ControllerType, TileType, BlockType>, BlockType extends MultiblockBlock<ControllerType, TileType, BlockType>> {
    ...
    public MultiblockController(@Nonnull World world, @Nonnull Validator<MultiblockTile<?, ?, ?>> tileTypeValidator, @Nonnull Validator<MultiblockBlock<?, ?, ?>> blockTypeValidator) {
        ...
        this.world = world;
        theinventorstech.addController(this);
    }
    ...
    final void attemptAttach(@Nonnull MultiblockTile<?, ?, ?> toAttachGeneric) {
        ...
        try {
            ...
            LOGGER.error("Tile type validator implemented wrong! Cleared tile that couldn't be cast. " + toAttachGeneric.getClass().toGenericString());
            return;
        }
        ...
        updateAssemblyAtTick = theinventorstech.tickNumber() + 1;
    }
    ...
    final void detach(@Nonnull TileType toDetach, boolean onChunkUnload, boolean checkForDetachments) {
        ...
        if (blocks.isEmpty()) {
            theinventorstech.removeController(this);
        }
    ...
    public final void update() {
        if (lastTick >= theinventorstech.tickNumber()) {
            return;
        }
        lastTick = theinventorstech.tickNumber();
        ...
        if (!controllersToMerge.isEmpty()) {
            ...
            for (ControllerType otherController : controllersToMerge) {
                theinventorstech.removeController(otherController);
...
//            blocks.forEach((pos, block) -> world.notifyNeighborsOfStateChange(pos, block.getBlockState().getBlock()));
    ...
    private void disassembledBlockStates() {
        ...
        blocks.forEach((pos, tile) -> {
            ...
            if (state != tile.getBlockState()) {
                newStates.put(tile.getBlockPos(), state);
                tile.clearCache();
            }
        });
        Util.setBlockStates(newStates, world);
